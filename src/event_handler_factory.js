...
function getEventHandler(event) {
  ...
  switch (eventType) {
    ...
    default:
      console.log(
        `Salesforce application state changed event handler has no handler for event type ${eventType}`
      );
      return () => {};
  }
}
...
function getStateChangedEvent(event) {
  ...
  if (toState === 'Approved, ARC') {
    ...
    if (toState !== 'Application Sent, University') {
      console.log(
        `Reverting application with Genesis ID ${genesisApplicationId} because the state changed from ${fromState} to ${toState}`
      );
      return handleRevertApprovalEvent.bind(event);
    }
  }
  console.log(
    `No handler available for state combination from_state:${fromState} to_state:${toState}.`
  );
  return () => {};
}
